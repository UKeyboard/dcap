...
config['network'] = dict(
    package = "model",
    module = ("ResNet12wider_wDCplus" if is_softlabel else "ResNet12wider_wDC") if is_dense else "ResNet12wider_wC",
    param = dict(
...
config["data"] = dict(
    package = "dataset",
    module = "MiniImagenetLMDBHorizontal",
    train = dict(
        version = 'protonet',
        phase = 'train', 
        transform_name =  'aug_train+' if is_augtrain_plus else 'aug_train',
        factor = 0.9,
        category_pool_name = "trainval",
        label_indent = 0,
