...
'experiments/basic_351w_tiered/basic_351w_tiered_conv64_augplus_multi_dense_hardlabel_wAC_wBias_woTBN_b512_e120_ckpt_epoch115.best', # dense pretrained with final activation
],[
# gap pre-trained models
None, # gap pretrained without final activation
'experiments/basic_351w_tiered/basic_351w_tiered_conv64_augplus_multi_avg_hardlabel_wAC_wBias_woTBN_b512_e120_ckpt_epoch115.best', # gap pretrained with final activation
]
]
...
if pretrained_model is not None:
    ...
    assert 'basic_351w_tiered_conv%d'%conv_out_dim in pretrained_model
else:
    is_meta_fintuning = False
#
max_num_epochs = 50 if is_meta_fintuning else 200


# build configure
config = {}
config["name"] = '_'.join([
    "basic_protonet_plus_5w5s_tiered", 
    'conv%d'%conv_out_dim,
